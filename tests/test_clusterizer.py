...
class TestClusterizer(unittest.TestCase):
    ...
    def test_exceptions(self):
        ...
        for pure_python in [False, True]:
            clusterizer = HitClusterizer(pure_python=pure_python)
            ...
            hit_dtype = np.dtype([('event_number', '<i8'),
                                  ...
                                  ('row', '<u2'),
                                  ('charge', '<u2')])
            clusterizer = HitClusterizer(hit_fields=hit_mapping, hit_dtype=hit_dtype, pure_python=pure_python)
            ...
            with self.assertRaises(ValueError):
                clusterizer = HitClusterizer(hit_fields=hit_mapping, hit_dtype=hit_dtype_new, pure_python=pure_python)
    ...
    def test_custom_hit_fields(self):
        for pure_python in [False, True]:
            ...
            clusterizer = HitClusterizer(hit_fields=hit_fields, hit_dtype=hit_dtype, pure_python=pure_python)
    ...
    def test_custom_cluster_fields(self):
        for pure_python in [False, True]:
            ...
            # Initialize clusterizer and cluster test hits with self defined data type names
            clusterizer = HitClusterizer(cluster_fields=cluster_fields, cluster_dtype=cluster_dtype, pure_python=pure_python)
            hits = create_hits(n_hits=10, max_column=100, max_row=100, max_frame=1, max_charge=2)
            hits_clustered, clusters = clusterizer.cluster_hits(hits)
